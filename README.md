...
表示核は産業機器のステータスランプ感覚へ寄せる。本プロジェクト固有の青は観測、黄は追加確認員を
呼ぶ、赤は停止・復旧対応能力を持つ体制が必要、灰黒は不明で執行不可を表す。色だけでなく状態語、
アイコン、音声を併記する。これは特定の産業規格へ準拠した色定義ではない。

その枯れた制御核へ、走査線、係数、オーラ、無機質音声等のSF外皮を被せる。SF演出の目的は、金勘定側が
現場状態へ興味を持ち、黄色で人を足し、赤になる前に消火体制を用意すること。係数の対象は人間ではなく
`operation_id + snapshot_id`であり、個人別犯罪係数、担当者ランキング、エンジニアへの責任転嫁は禁止する。

本GUIの特徴は、測定後の故障表示ではなく**測定前**に出ることにある。`time_scope=PROSPECTIVE`を明示し、
まだ観測していないRiskとBenefitを`PROJECTED / INFERRED / UNKNOWN`として並べる。AIが計画、回路、
過去成果物から未来の作業を先に可視化し、決裁側へ人員、装備、時間、停止・復旧能力の配備を促す。

安全は責任の押し付け先ではなく、事前に組織能力を組み上げる工学対象として扱う。技術側は必要controlを
定義し、金勘定側は金で防げる事故へ人員・装備・時間を供給し、実行側は承認済み条件で操作する。
詳細は [`docs/architecture/prospective-meta-safety.md`](docs/architecture/prospective-meta-safety.md)を参照。
上位のLayer B哲学正本はZeroRoomLab-manifestの
[ちくわ砲 — 幻想化によるProspective Meta-Safety](https://github.com/saitoomituru/ZeroRoomLab-manifest/blob/main/docs/philosophy/chikuwa-cannon-prospective-meta-safety.ja.md)に置く。

安全を成立させる二つの半分として、原初の神話・夢・ロマン・哲学を矮小化しないことと、それを十分な
安全率で執行する人員・時間・設備・停止復旧余裕を別々に扱う。目的だけが強く資源が不足する状態は
やりがい搾取警報、資源があって目的が消えた状態は目的関数ドリフトとして表示する。個人の忠誠心や
熱意はscore化しない。Layer A契約は
[`docs/architecture/purpose-function-ledger.md`](docs/architecture/purpose-function-ledger.md)、Layer B正本は
[神話的士気管理と目的関数矮小化連鎖](https://github.com/saitoomituru/ZeroRoomLab-manifest/blob/main/docs/philosophy/mythic-morale-and-purpose-attenuation.ja.md)を参照。
...
> roles, staffing needs, and reason-coded risk/benefit signals. Technical gates remain authoritative,
...
> Color and science-fiction styling describe an operation and its current context, never a person's worth
> or liability. The underlying status-light semantics remain usable without the theme.
> Prospective colors describe a proposed future operation, not an accident already in progress. Safety is
> treated as an engineered organizational capability assembled before measurement.
...
| Season 0 | 実験準備中 | SDS1204X-EのmDNS観測、識別、ペアリング台帳、原本取得、再表示の縦切り |
