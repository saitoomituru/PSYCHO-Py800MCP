...
- [ ] 最初の`*IDN?`用BootstrapRunbookの対象機器、完全一致byte、上限、timeout、中止条件を確定する
- [ ] bootstrap観測後に実装するquery-only ApprovalSessionのscope、TTL、失効条件を確定する
- [ ] Git外のRunArtifact保存先と公開用正規化経路を確定する
- [ ] Season 0最初の実験ノートを`notes/experiment-template.md`から作成する
- [ ] 人間が固定runbookを明示起動した場合だけ一回限り開くbootstrapゲートを別コミットにする
