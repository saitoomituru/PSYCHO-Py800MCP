...
- 限定周回を`ACQUISITION_STOPPED -> DUT_RESTART_REQUESTED -> SETTLING -> AUTOSET_BOUNDED ->
  ENVELOPE_VALIDATION`として記録する。
- AUTO回数、optimizer反復数、settle時間はhardcodeせず、規格、社内SOP、機種能力、実験目的から導出し、
  ユーザーが承認する。
- 複数channel／登録機材の利得・伝達特性探索は`OptimizationEnvelope`内で自動化する。
- `AnomalyContract`へ異常定義、停止順序、ログ退避、部分確定、次計画へのevidence引継ぎを固定する。
- 承認済みStage／OptimizationEnvelope内のRUN、re-arm、source探索は毎点承認へ戻さない。
