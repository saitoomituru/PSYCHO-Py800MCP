...
推奨local pathはGit外の`state/private/instrument-pairing.sqlite3`とする。private/local IP、MAC、serial、
機種情報、通常logは`LOCAL_BENCH`内のoperational metadataとしてSQLiteへ保存してよい。高度暗号化を
初期必須条件にせず、OS account permission、transaction、backup、integrity checkを基本controlにする。
schema migration versionと生成アプリversionを保持する。

credential、token、client certificateが登場した場合だけOS keychain等へ分離し、DBにはreferenceを置く。
内部保存と公開exportは別policyとし、生DB、未匿名化serial、MACを公開Gitへ自動commitしない。local segmentの
公開記録はユーザー方針に従う。詳細は
...
- local IP、model、firmware、通常logをcredentialとして過剰秘匿しない
