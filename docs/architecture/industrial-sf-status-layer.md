...
このstatus lampは主として測定前の未来予告に使う。`PROSPECTIVE` badgeと破線frameを併記し、実機の
現在故障alarmと混同させない。
...
全snapshotは`time_scope=PROSPECTIVE | LIVE | POST_RUN`を持つ。測定前係数は`PROSPECTIVE`であり、
事故発生や実測済みriskを意味しない。RiskとBenefitには`PROJECTED`、`INFERRED`、`UNKNOWN`を付け、
未来予測を`OBSERVED`と書かない。
...
yellow／redは、危険へ向かう前に必要な組織能力を要求する`RiskCapacityRequest`を生成する。

- roles: reviewer、Engineer、stop authority、incident coordinator
- equipment: probe、isolation、保護具、代替instrument等の必要カテゴリ
- time: review、setup、recoveryに必要なeffort band
- recovery: safe stop、復旧、部分成果物確定、incident handling能力

金額は算出せず、decision layerがstaff／fund／deferできるrole・equipment・effort要求へする。
