...
複数channel演算、登録機材を組み合わせた利得・伝達特性評価、optimizer探索は一律blockしない。
`OptimizationEnvelope`へ各channel、source、DUT、負荷、common-mode、GNDのhard constraintを固定し、
その内側ではAIが探索点と観測窓を選べる。AUTO回数や反復数はhardcodeせず、採用規格、社内SOP、機種能力、
実験目的から導出した値をユーザーが承認する。

異常signalと異常後の縮退は`AnomalyContract`として事前承認する。異常の定義、判定窓、停止順序、raw data、
設定snapshot、command log、部分成果物、handoff、定義外異常のfallbackを固定する。異常成立後は新しい探索を
止め、証拠を次の測定計画へ渡す。表示は「誰のせいか」でなく、設計想定、現物確認、自動制約のどこで逸脱を
検出したかを示す。

各channelのrange承認は`ChannelExposureContract`として、Vrms、Vpeak、DC、offset、transient、probe、BNC到達値、
coupling、impedance、測定点、referenceを個別に持つ。極端に不適切なV/divは`DISPLAY_RANGE_MISMATCH`、
probe／BNC／common-mode定格の超過は`PHYSICAL_INPUT_LIMIT`として分離する。小さいV/divが即物理破損という
誤説明も、画面に収まるV/divなら安全という誤説明もしない。

channel個別承認だけでは共有GNDの安全を証明しない。`SharedReferenceContract`で全channelのground lead、
earth reference、差動／floating定格、絶縁境界を組合せ検証する。preamp、真空管plate、output transformer二次側を
同じ「amplifier」rangeへまとめず、物理nodeごとに承認する。

接地graphを成立させる責務は物理作業側のEngineer Checkに置く。絶縁transformerの位置、scope内部CH共通GND、
入力信号源とDUTのground、差動probeの定格、通信線等の再接地pathを`EngineerProbingDeclaration`へ記録する。
AIは欠落・矛盾を検出して自動writeを閉じるが、現物を見たことにしない。すでに人間が成立させた波形のreadは
止めず、未記録条件を`UNKNOWN`として解析結果へ伝播する。

ユーザーはMeasurementPlanの最終approve／reject権限を持つ。channel range、probe倍率、GND、coupling、impedance、
測定点、異常退避flowが意図または現物と合わなければ`REJECTED_FOR_REPLAN`を返す。AIはreject理由を消さず、
新しいplan hashを作って再提出する。承認後にこれらが変わればwrite bindingだけを失効させ、既存dataのreadまで
停止しない。
...
- 複数CH／複数機材optimizerが承認済みhard constraintを越えない
- AUTO回数、反復数、異常退避flowをhardcodeせず、根拠付きplanとして承認する
- channel個別rangeと全channel共有referenceの両方を承認し、V/divと物理耐圧を混同しない
- EngineerProbingDeclarationなしにAIが絶縁、内部共通GND、差動probe成立を推測しない
- ユーザーreject後に旧planのwrite権限を残さず、新plan hashで再提出する
