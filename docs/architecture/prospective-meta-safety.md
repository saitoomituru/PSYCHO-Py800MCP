...
[ちくわ砲 — 幻想化によるProspective Meta-Safety](https://github.com/saitoomituru/ZeroRoomLab-manifest/blob/main/docs/philosophy/chikuwa-cannon-prospective-meta-safety.ja.md)
...
本書はそのLayer A写像であり、Layer B本文を複製しない。
...
Prospective ScanはRiskとBenefitに加え、目的を保つ意味の半分と、その目的を十分な安全率で執行する
資源の半分を独立に確認する。神話、夢、ロマン、哲学は目的関数を保つ正本参照になり得るが、人員、
時間、設備、予算、停止・復旧能力の代替にはならない。

`PurposeIntegritySignal`と`ResourceIntegritySignal`を同一snapshotへ持たせる。目的が保持され資源が
不足する場合は`EXPLOITATION_RISK`、資源があって目的が減衰する場合は`PURPOSE_DRIFT`として表示する。
詳細契約は[`purpose-function-ledger.md`](purpose-function-ledger.md)を参照する。
...
幻想themeはlive warning、現在課題、実測事実を軽くせず、覆わず、muteせず、置換しない。幻想化は
`PROSPECTIVE`を直感表示するpresentation typeであり、現実paneと同時に存在する。
...
Project myth and purpose never substitute for compensation, staffing, time, equipment, recovery capability,
or engineering safety margins.
