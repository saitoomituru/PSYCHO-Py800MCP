...
- PurposeIntegritySignal: preserved / attenuated / severed / unknown
- ResourceIntegritySignal: margin recorded / reduced / insufficient / unknown
- Purpose Function Ledger IDとstage別変更event
...
Dashboardは、原初の夢・哲学・期待を福利厚生語へ薄めず、PurposeIntegritySignalとして現在stepとの接続を
示す。同時に、その目的へ十分な安全率を与える人員、時間、設備、停止・復旧余裕を
ResourceIntegritySignalとして別表示する。

- purpose high / resource sufficient: 目的と実行条件が接続している候補
- purpose high / resource insufficient: `やりがい搾取警報`
- purpose low / resource sufficient: `目的関数ドリフト`
- purpose low / resource insufficient: 上流へ差し戻し、現場の気合で埋めない

この表示は個人の士気や忠誠心を測らない。不満、恐れ、疲労、異議を削除するのでなく、目的または資源の
欠損を再確認する導線へ変換する。詳細は[`purpose-function-ledger.md`](purpose-function-ledger.md)を参照。
...
│ purpose: PRESERVED  resource: INSUFFICIENT   │
│ warning: ロマンを不足資源の代用にしない       │
