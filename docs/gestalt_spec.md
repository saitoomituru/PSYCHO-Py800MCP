...
- 必須: 決定役割、RiskExposure、BenefitProjection、法的賠償責任の状態を別項目で表示する
...
非専門の意思決定者向けには、次を同じ画面へrangeで出す。単一のそれらしい金額へ丸めない。

- instrument / probe / DUT replacement
- downtimeとlab interruption
- retest、再校正、失われるデータとschedule
- incident response、医療・救急対応の運用費
- third-party property / service impact
- 追加人員と追加確認に必要な費用

各値はcurrency、low/high、算定根拠、入力者、時点、confidenceを持つ。金額がない場合はゼロではなく
`UNKNOWN`。これは損失可能性のプレゼンテーションであり、法的賠償額、保険支払額、会計引当を意味しない。

### ベネフィット表示

RiskExposureと同じ画面に、欠陥検出、手戻り回避、開発学習、測定証拠、schedule短縮、再利用性、
milestone・事業上のoption等を`BenefitProjection`としてrangeと根拠付きで表示する。Benefitも単一の
期待金額へ無理に変換せず、非金銭価値と不確実性を保持する。
...
暫定GUI profileでは両方を0〜300へ正規化し、profile IDと算出理由を必ず表示する。旧0〜500式とは
