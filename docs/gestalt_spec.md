...
- 禁止: 係数、色、オーラをperson ID、個人評価、ランキング、賠償主体へ付与する
...
### project-local status lamp

| lamp | 状態語 | 当直向け一言 | 組織action |
|---|---|---|---|
| blue | 観測・記録支援中 | 見ているだけ | 現体制で継続 |
| yellow | 追加確認員要請 | 一人足す | review／現物確認を配備 |
| red | 高危険度体制要請 | 後始末できる人を先に呼ぶ | stop authority、復旧・incident対応能力を配備 |
| gray-black | 不明・執行不可 | 材料不足 | unknown解消まで新規執行block |

この色mappingは本プロジェクト固有で、特定の産業規格準拠を主張しない。色だけに依存せず、状態語、
形状、アイコン、音声を併記する。lampはoperation snapshotへ付き、人間へ付かない。
