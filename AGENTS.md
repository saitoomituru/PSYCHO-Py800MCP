...
すべての実機通信は、対象機器、許可操作、上限、TTL、解除条件を持つApprovalSessionの内側で行う。
Phase 0では実験ゲートを閉じ、実機通信を実装・実行しない。query-only実験も別の明示承認後に開始する。
...
src/psycho_py800mcp/*_mcp_server.py - FastMCPサーバー（公式`mcp` packageとの名前衝突を避ける）
...
- ApprovalSessionなしにSCPIコマンドを実行しない
- unknownまたは観測不能を許可・成功へ変換しない
